...
public class GroupeCapaciteAccueil {
  ...
  public static final String JSON_PROPERTY_RESTANTE = "restante";
  ...
  /**
   * Return true if this Groupe_capaciteAccueil object is equal to o.
   */
  @Override
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `totale` to the URL query string
    if (getTotale() != null) {
      joiner.add(String.format("%stotale%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getTotale()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `restante` to the URL query string
    if (getRestante() != null) {
      joiner.add(String.format("%srestante%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getRestante()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
